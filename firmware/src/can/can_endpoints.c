...
uint8_t avlos_controller_current_max_Iq_feedback(uint8_t * buffer, uint8_t * buffer_len, Avlos_Command cmd)
{
    if (AVLOS_CMD_READ == cmd) {
        float v;
        v = controller_get_max_Iq_feedback();
        *buffer_len = sizeof(v);
        memcpy(buffer, &v, sizeof(v));
        return AVLOS_RET_READ;
    }
        ...
        float v;
        memcpy(&v, buffer, sizeof(v));
        controller_set_max_Iq_feedback(v);
        return AVLOS_RET_WRITE;
    }
    return AVLOS_RET_NOACTION;
}
