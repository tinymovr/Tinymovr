...
{
if (AVLOS_CMD_READ == cmd) {
        return _avlos_getter_string(buffer, buffer_len, system_get_fw_version_string);
    }
    return AVLOS_RET_NOACTION;
}
...
uint8_t avlos_nvm_num_slots(uint8_t * buffer, uint8_t * buffer_len, Avlos_Command cmd)
{
if (AVLOS_CMD_READ == cmd) {
        uint8_t v;
        v = nvm_wl_get_num_slots();
        *buffer_len = sizeof(v);
        memcpy(buffer, &v, sizeof(v));
        return AVLOS_RET_READ;
    }
...
uint8_t avlos_nvm_current_slot(uint8_t * buffer, uint8_t * buffer_len, Avlos_Command cmd)
{
if (AVLOS_CMD_READ == cmd) {
        uint8_t v;
        v = nvm_wl_get_current_slot();
        *buffer_len = sizeof(v);
        memcpy(buffer, &v, sizeof(v));
        return AVLOS_RET_READ;
    }
...
uint8_t avlos_nvm_write_count(uint8_t * buffer, uint8_t * buffer_len, Avlos_Command cmd)
{
if (AVLOS_CMD_READ == cmd) {
        uint32_t v;
        v = nvm_wl_get_write_count();
        *buffer_len = sizeof(v);
        memcpy(buffer, &v, sizeof(v));
        return AVLOS_RET_READ;
    }
...
uint8_t avlos_nvm_remaining_writes_estimate(uint8_t * buffer, uint8_t * buffer_len, Avlos_Command cmd)
{
if (AVLOS_CMD_READ == cmd) {
        uint32_t v;
        v = nvm_wl_get_remaining_writes_estimate();
        *buffer_len = sizeof(v);
        memcpy(buffer, &v, sizeof(v));
        return AVLOS_RET_READ;
    }
...
uint8_t avlos_controller_set_pos_vel_setpoints(uint8_t * buffer, uint8_t * buffer_len, Avlos_Command cmd)
{
    ...
    float ret_val = controller_set_pos_vel_setpoints_user_frame(pos_setpoint, vel_setpoint);
